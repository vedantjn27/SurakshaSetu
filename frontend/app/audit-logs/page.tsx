...
interface AuditLog {
  id: string;
  timestamp: string;
  actor: string;        // performed_by / actor
  action: string;       // decision_type
  entity_type: string;
  ubid?: string;
  outcome?: string;
...
export default function AuditLogsPage() {
  ...
  const getActionColor = (action: string) => {
    ...
    if (act.includes('update') || act.includes('override')) return 'bg-blue-100 text-blue-800';
    if (act.includes('delete')) return 'bg-red-100 text-red-800';
    if (act.includes('view') || act.includes('access')) return 'bg-gray-100 text-gray-800';
            ...
            {loading ? (
              ...
              logs.map((log) => (
                ...
                >
                  ...
                  <td className="p-4 text-sm text-foreground">{log.ubid || log.entity_type}</td>
